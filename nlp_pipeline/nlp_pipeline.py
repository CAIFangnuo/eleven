...
def pre_processing(df, title_col, review_col):
    df['all_review'] = df['review_title'] + df['review_text']
    df['all_review'] = df['all_review'].apply(lambda x: simple_preprocess(x,deacc=True))
    df['preprocess_data'] = df['all_review'].apply(' '.join)
    df['preprocess_data'] = df['preprocess_data'].apply(lambda x: remove_stopwords(x))
    return df
...
def generate_topic_sentiment(df, title_col, review_col):
    ...
    df['topic_sentiment'] = df['preprocess_data'].apply(lambda x: feature_sentiment(x, pos, neg))
    return df
...
def main():
    ...
    save_path = os.path.join(os.path.dirname(args.path), f"nlp_pipeline_{os.path.basename(args.path)}")
